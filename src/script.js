...
function changeDirection() {
    ...
    const threshold = touchmiddlex;
    const swipeMin = 20; // minimum px to register as intentional swipe

    if (Math.abs(delta) < swipeMin) return; // ignore accidental micro-swipes

    if (delta > threshold) {
        // Swiped left (far) → slide 3
        currentSlide = 2;
    } else if (delta > 0) {
        // Swiped left (short) → slide 2
        currentSlide = 1;
    } else if (delta < -threshold) {
        // Swiped right (far) → slide 1
        currentSlide = 0;
    } else {
        // Swiped right (short) → slide 2
        currentSlide = 1;
    }
